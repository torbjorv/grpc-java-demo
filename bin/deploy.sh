...
APIVERSION=`gcloud service-management describe myapi-grpc-java.endpoints.$PROJECTID.cloud.goog | grep -o 'id:.*' | cut -c5-`

# delete previous deployment
kubectl delete deploy myapi-grpc-java
kubectl delete service myapi-grpc-java-loadbalancer
kubectl delete configmap nginx-config

docker build -t=gcr.io/$PROJECTID/myapi-grpc-java:latest --rm .
gcloud docker push gcr.io/$PROJECTID/myapi-grpc-java:latest
...
docker build -t=gcr.io/$PROJECTID/myapi-grpc-swaggerui:latest --rm . -f swaggerui.Dockerfile
gcloud docker push gcr.io/$PROJECTID/myapi-grpc-swaggerui:latest
